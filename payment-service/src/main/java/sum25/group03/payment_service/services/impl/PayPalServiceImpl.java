...
//                    request.getAmount(),
//                    "https://079fe25a79d4.ngrok-free.app/api/paypal/return",
//                    "https://079fe25a79d4.ngrok-free.app/api/paypal/cancel"
//            );
//
...
import lombok.extern.slf4j.Slf4j;
import org.springframework.stereotype.Service;
import sum25.group03.payment_service.clients.PayPalClient;
...
public class PayPalServiceImpl implements PayPalService {
    ...
    public String createPayment(PaymentRequestRequest request) {
        try {
            ...
            // Call PayPal API via client
            String response = payPalClient.createOrder(body);
            ...
            log.info("PayPal order created: {}", json.get("id").asText());
            return approveUrl;
    ...
    public String capturePayment(String orderId) {
        try {
            String response = payPalClient.captureOrder(orderId);
    ...
    public String getOrderStatus(String orderId) {
        try {
            ...
            JsonNode json = objectMapper.readTree(details);
            String status = json.get("status").asText();
            log.info("PayPal order status: id={}, status={}", orderId, status);
            return status;

        } catch (Exception e) {
            log.error("Error getting PayPal order status", e);
            throw new RuntimeException("Failed to get PayPal order status", e);
        }
    }
}